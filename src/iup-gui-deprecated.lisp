...
(defun ad/stroke-coords-2a (coords-2a canvas path-mode)
  (let* ((dims (array-dimensions coords-2a))
	 (nrows (car dims)))
...
(defmethod ad/stroke (element canvas)
  (ad/stroke-coords-2a (ad/coords element)
		       canvas
...
;; (defmethod ad/stroke ((element <path>) canvas)
;;   (let* ((path-width (path-width element)))
;;     (multiple-value-bind
;; 	  (x y)
;; 	(wd:world-to-canvas canvas path-width path-width)
;;       (setf (cd:line-width canvas) (min x y))
;;       (setf (cd:line-join canvas) :join-miter)
;;       ;;      (setf (cd:write-mode canvas) :write-xor)
;;       (setf (cd:foreground canvas)
;; 	    (cd:encode-alpha cd:+dark-cyan+ 50))
;;       (call-next-method)
;;       (setf (cd:foreground canvas) cd:+black+)
;;       (setf (cd:line-width canvas) 1)
;;       (setf (cd:write-mode canvas) :write-replace))))


(defun ad/stroke-ref (element canvas) 
  (let ((coords (ad/coords element)))
    (wd:mark canvas (aref coords 0 0) (aref coords 0 1))))
...
(defun ad/min-max-bounds-to-cd-bounds (array2x2)
  (list (aref array2x2 0 0)
	(aref array2x2 1 0)
	(aref array2x2 0 1)
	(aref array2x2 1 1)))
